...
async function main() {
  ...
  const backendUrl = backend?.service?.serviceDetails?.url 
    || (backend?.service?.slug ? `https://${backend.service.slug}.onrender.com` : null);
  console.log('\nBackend URL:', backendUrl);

  if (!backendUrl || backendUrl.includes('undefined')) {
    console.error('Failed to get backend URL. Aborting panel creation.');
    return;
  }
  ...
  const panelUrl = panel?.service?.serviceDetails?.url 
    || (panel?.service?.slug ? `https://${panel.service.slug}.onrender.com` : 'unknown');
  console.log('\n========================================');
