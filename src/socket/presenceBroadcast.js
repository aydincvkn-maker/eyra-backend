...
const emitPresenceUpdateToVisibleSockets = (targetPresence) => {
  ...
  const status =
    targetPresence?.status || (targetPresence?.online ? "online" : "offline");
  const lastSeen = targetPresence?.lastSeen || Date.now();

  // ✅ FIX: Send full payload with presenceStatus, isOnline, isLive etc.
  // Flutter clients read these fields — the old minimal payload was missing them.
  const payload = {
    userId,
    status,
