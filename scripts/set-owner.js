...
(async () => {
  ...
  try {
    ...
    const prevOwners = await User.find({ isOwner: true })
      .select("email")
      .lean();
    if (prevOwners.length > 0) {
      console.log(
        `📋 Mevcut owner'lar temizleniyor:`,
        prevOwners.map((u) => u.email).join(", "),
      );
    ...
    console.log(
      `✅ ${user.email} (${user.username}) artık PATRON (owner) olarak atandı!`,
    );
